...
if __name__ == '__main__':
    ...
    path1  = "/tera04/zhwei/PCSE/data/atm"
    ...
    for name,idx,colr in zip(names,idxs,colors):
        for ssp in ssps:
                ####
                amt0=xr.open_dataset(f'{path1}/{ssp}/PCSE_input.nc')
                amt1=(amt0.where((amt0.time.dt.month>4)&(amt0.time.dt.month<12)&(amt0.time.dt.year > 2014)&(amt0.time.dt.year < 2100), drop=True)).where(crop == idx, drop=True)
                ...
                ds1 = xr.open_dataset(f'{path}/default/{name}_output_{ssp}_default.nc')["TAGP"]
                ds1 = (ds1.where((ds1.time.dt.month>4)&(ds1.time.dt.month<12)&(ds1.time.dt.year > 2014)&(ds1.time.dt.year < 2100), drop=True)).where(crop == idx, drop=True)
                ds1Yield = ds1.groupby("time.year").max("time").groupby('year').mean(...)


                

                ds2 = xr.open_dataset(f'{path}/co2/{name}_output_{ssp}_co2.nc')["TAGP"]
                ds2 = (ds2.where((ds2.time.dt.month>4)&(ds2.time.dt.month<12)&(ds2.time.dt.year > 2014)&(ds2.time.dt.year < 2100), drop=True)).where(crop == idx, drop=True)
                ds2Yield = ds2.groupby("time.year").max("time").groupby('year').mean(...)
                
                ds3 = xr.open_dataset(f'{path}/precipitation/{name}_output_{ssp}_precipitation.nc')["TAGP"]
                ds3 = (ds3.where((ds3.time.dt.month>4)&(ds3.time.dt.month<12)&(ds3.time.dt.year > 2014)&(ds3.time.dt.year < 2100), drop=True)).where(crop == idx, drop=True)
                ds3Yield = ds3.groupby("time.year").max("time").groupby('year').mean(...)

                ds4 = xr.open_dataset(f'{path}/temperature/{name}_output_{ssp}_temperature.nc')["TAGP"]
                ds4 = (ds4.where((ds4.time.dt.month>4)&(ds4.time.dt.month<12)&(ds4.time.dt.year > 2014)&(ds4.time.dt.year < 2100), drop=True)).where(crop == idx, drop=True)
                ...
                con_co2=abs(sen_co2*slope_co2)/(abs(sen_tas*slope_tas)+abs(sen_co2*slope_co2)+abs(sen_preciptation*slope_precip))*100.0   #*100  #./slope_Yileld
                con_preciptation=abs(sen_preciptation*slope_precip)/(abs(sen_tas*slope_tas)+abs(sen_co2*slope_co2)+abs(sen_preciptation*slope_precip))*100.0
    ...
    df.to_csv('/tera04/zhwei/PCSE/data/output/sensitivity/contribution.csv')
