...
print("Syear: %s" % Syear)
print("Eyear: %s" % Eyear)
fname = os.path.join(data_dir, "output/sensitivity/co2/")
print("output file: %s" % fname)
print("**********************")
print(" ")


def FilePrapare(metrodata_dir):
    # get data ready!
    VarFile = ('%s/PCSE_input.nc' % (metrodata_dir))
...
- {Nyear}-03-01: null
  """
    agromanagement = yaml.safe_load(agro_soybean.format(year=year, Nyear=year + 1, crop='soybean'))
...
def Initial(data_dir, Start_time, dco2, ds):
    ...
    crop_data.set_active_crop('soybean', 'Soybean_VanHeemst_1988')
        ...
        for ilat in enumerate(ds.lat.values):
            ...
            if da.tasmax.values[0] > -9999.0:
                ...
                for i, inputs in enumerate(all_runs):
                    ...
                    sited = WOFOST80SiteDataProvider(WAV=10, CO2=dco2.loc[2015].values, SMLIM=0.35) #, NAVAILI=200.0,PAVAILI=200.0,KAVAILI=200.0
...
def core_cal(ds, ilonn, ilatn, temp, Start_time, dco2):
    ...
    if ds['tasmax'].values[5, ilatn, ilonn] > -9999.0:
        ...
        for i, inputs in enumerate(all_runs):
            ...
            # m = int(S_time[i])
            # Stime = ds.time[m]
            # start = (str(Stime.values)[0:10])
            # print("       simulation year: %s" %start)
            agromanagement = agro(year)
            print(year)
            sited = WOFOST80SiteDataProvider(WAV=10, CO2=dco2.loc[2015].values, SMLIM=0.35) #, NAVAILI=150, PAVAILI=50, KAVAILI=150
            parameters = ParameterProvider(cropdata=crop_data, soildata=soild, sitedata=sited)
...
if __name__ == '__main__':
    ...
    for ssp in ssps:
        ...
        Start_time = xr.open_dataset('/tera01/xuqch3/Dongbei/pr_t/pr_t_Growthday_%s.nc' % (ssp)).strat
        print(">>>>time prepared  done")

        ds = FilePrapare('/tera04/zhwei/PCSE/data/atm/%s' % (ssp))
        ...
        dk.to_netcdf('%s/soybean_output_%s_%s.nc' % (fname,ssp,runname), engine='netcdf4')
