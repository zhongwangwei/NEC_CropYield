...
print("Syear: %s" % Syear)
print("Eyear: %s" % Eyear)
#data_dir1 = os.path.join(os.getcwd(), "/tera01/xuqch3/PCSE/sensitivity/")
fname = os.path.join(data_dir, "output/adaptation/sowing/")
...
def FilePrapare(metrodata_dir):
    ...
    # VarFile=('%s/PCSE_input_ssp585.nc'%(metrodata_dir))
...
- {Nyear}-03-01: null
  """
    agromanagement = yaml.safe_load(agro_soybean.format(year=year, Nyear=year + 1, crop='soybean', startdate=start))
...
if __name__ == '__main__':
    ...
    for ssp in ssps:
        ...
        Start_time = xr.open_dataset('/tera04/zhwei/PCSE/data/pr_t/pr_t_Growthday_%s.nc' % (ssp)).strat
        print(">>>>time prepared  done")
        ds = FilePrapare('/tera04/zhwei/PCSE/data/atm/%s' % (ssp))
        for i in np.arange(80,147,7):
            Start_time=Start_time+float(i)
            ...
            dk.to_netcdf('%s/soybean_output_%s_%s_%s.nc' % (fname, ssp, runname,i), engine='netcdf4')
